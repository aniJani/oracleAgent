...
class ChainClient:
    def __init__(self, rest_client, host_account, contract_address, price_per_second, transaction_lock):
    ...
    async def register_on_chain_if_needed(self, specs: Dict[str, Any]) -> None:
        ...
        if existing:
            logger.info("Host already has listing(s) on-chain. Skipping registration.")
            return
        ...
        logger.info(f"🔗 Registering '{specs['identifier']}' on-chain using {function_name}...")
    ...
    async def claim_payment_for_job(self, job_id: int):
        while job_id in self.active_jobs:
            ...
            try:
                ...
                logger.info(f"DEBUG: Parsed response from get_job for Job ID {job_id}: {parsed}")
                ...
                if accrued <= claimed_amount:
                    if claim_timestamp >= max_end_time:
                        logger.info(f"Job {job_id}: fully claimed or at end; stopping claims.")
                        self.stop_container_callback(job_id)
                        self.active_jobs.discard(job_id)
                        return
                    logger.info(f"Job {job_id}: nothing new to claim yet.")
                    await asyncio.sleep(PAYMENT_CLAIM_INTERVAL_SECONDS)  # noqa: F821 (asyncio imported in agent)
                    continue
                final_session_duration = 0
                if claim_timestamp >= max_end_time:
                    start_ts = self.session_start_times.get(job_id) if self.session_start_times else None
                    if start_ts:
                ...
                try:
                    ...
                    if "NUMBER_OF_ARGUMENTS_MISMATCH" in str(e):
                        logger.warning("Older claim_payment ABI; retrying with 2 arguments.")
                        ...
                        await self.submit_transaction(payload2, f"Successfully claimed payment (2-arg ABI) for Job ID {job_id}")
                    else:
                        raise
                if claim_timestamp >= max_end_time:
                    logger.info(f"Job {job_id}: final claim submitted; stopping session.")
                    self.stop_container_callback(job_id)
                    self.active_jobs.discard(job_id)
                    return
            except ApiError as e:
                logger.error(f"API Error claiming payment for Job ID {job_id}: {e}", exc_info=True)
                self.stop_container_callback(job_id)
                self.active_jobs.discard(job_id)
                return
            except Exception:
                logger.error(f"Generic error claiming payment for Job ID {job_id}", exc_info=True)
                self.stop_container_callback(job_id)
                self.active_jobs.discard(job_id)
                return
